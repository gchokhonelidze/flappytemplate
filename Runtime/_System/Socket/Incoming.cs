...
namespace FlappyTemplate
{
    ...
    public class Incoming : MonoBehaviour
    {
                ...
                if (eventName == nameof(EGameEvent.ON_GROUP))
                {
                    ...
                    foreach (var element in values.OfType<JObject>())
                    {
                        var groupEventName = element.Value<string>(
                            nameof(PayloadGroupElementDto.E)
                        );
        ...
        private void ProcessDataEvent(string eventName, string dataJson)
        {
                ...
                switch (eventName)
                {
                    ...
                    case nameof(EGameEvent.ON_STATE):
                    {
                        ...
                        stateManager.MainState._GameState =
                            stateManager.MainState.GameState.ToGeneric();
                    ...
                    case nameof(EGameEvent.ON_IND_STATE):
                    {
                        ...
                        stateManager.MainState._IndState =
                            stateManager.MainState.IndState.ToGeneric();
                    ...
                    {
                        var betInfoIncreaseDto = Utils.TryDeserialize<BetInfoIncreaseDto>(dataJson);
                        ...
                        var bet = betInfo.Bets.FirstOrDefault(el =>
                            el.BetId == betInfoIncreaseDto.BetId
                        );
                        if (bet is null)
                            return;
                    ...
                    {
                        var seedDto = Utils.TryDeserialize<SeedDto>(dataJson);
                        if (seedDto is null)
                            return;
                    ...
                    {
                        var statsDto = Utils.TryDeserialize<StatsDto>(dataJson);
                        if (statsDto is null)
                            return;
                    ...
                    case nameof(EGameEvent.ON_FREESPIN):
                    {
                        ...
                        {
                            stateManager.Events.OnFreespin?.Invoke(
                                stateManager.MainState.FreespinInfo
                            );
                        }
                        break;
                    }
                    ...
                    {
                        var arr = Utils.TryDeserialize<HistoryDto[]>(dataJson);
                    ...
                    {
                        var dto = Utils.TryDeserialize<Dictionary<string, JToken>>(dataJson);
                        stateManager.MainState.Settings = stateManager.MainState.Settings.MergeFull(
                            dto
                        );
                        stateManager.MainState._Settings =
                            stateManager.MainState.Settings.ToGeneric();
                    ...
                    {
                        var dto =
                            Utils.TryDeserialize<Dictionary<string, JToken>>(dataJson)
                            ?? new Dictionary<string, JToken>();
                        stateManager.MainState.SharedData =
                            stateManager.MainState.SharedData.MergeFull(dto);
                        stateManager.MainState._SharedData =
                            stateManager.MainState.SharedData.ToGeneric();
                    ...
                    case nameof(EGameEvent.ON_MULTI_INIT):
                    {
                        ...
                        var dto = Utils.TryDeserialize<MultiInitDto>(dataJson);
                        if (dto is null)
                        {
                        ...
                        Debug.Log(
                            $"ON_MULTI_INIT rooms count: {stateManager.MultiState.MultiRooms?.Length ?? 0}"
                        );
                    ...
                    {
                        var dto = Utils.TryDeserialize<MultiJoinTableDto>(dataJson);
                        stateManager.MultiState.MultiLocation = dto.Location;
                        stateManager.MultiState.MultiMySeatIndex = dto.SeatIndex;
                        if (dto.MyRoom != null)
                        {
                    ...
                    {
                        var dto = Utils.TryDeserialize<MultiLeaveTableDto>(dataJson);
                        ...
                        else
                        {
                            ...
                            stateManager.MainState._IndState.Clear();
                    ...
                    {
                        var dto = Utils.TryDeserialize<MultiTakeSeatDto>(dataJson);
                    ...
                    {
                        var dto = Utils.TryDeserialize<MultiLeaveSeatDto>(dataJson);
                    ...
                    {
                        var dto = Utils.TryDeserialize<MultiTurnUpdateDto>(dataJson);
                        stateManager.MainState.GameState =
                            stateManager.MainState.GameState.MergeFull(dto.SPubPartial);
                        stateManager.MainState._GameState =
                            stateManager.MainState.GameState.ToGeneric();
                        stateManager.MultiState.MultiTurnIndex = dto.TurnIndex;
                        stateManager.MultiState.MultiTurnId = dto.TurnId;
                        ...
                        stateManager.MultiState.MultiTurnPlayers = dto
                            ...
                            {
                                var seatIndex =
                                    stateManager.MultiState.MultiSeatsInRound.Keys.FirstOrDefault(
                                        seatIndex =>
                                            stateManager.MultiState.MultiSeatsInRound[seatIndex].Id
                                            == id
                                    );
                                return new MultiTurnPlayerDto
                                {
                                    SeatIndex = seatIndex,
                                    Player = stateManager.MultiState.MultiSeatsInRound.TryGetValue(
                                        seatIndex,
                                        out var player
                                    )
                                        ? player
                    ...
                    {
                        var dto = Utils.TryDeserialize<MultiLeftDto>(dataJson);
                    ...
                    {
                        var dto = Utils.TryDeserialize<MultiStartRoundDto>(dataJson);
                        stateManager.MultiState.MultiTurnIndex = dto.TurnIndex;
                        stateManager.MultiState.MultiSeatsTaken = dto.SeatsTaken;
                    ...
                    {
                        var dto = Utils.TryDeserialize<ChipBalanceDto>(dataJson);
                    ...
                    {
                        var dto = Utils.TryDeserialize<ChipBalanceInGameDto>(dataJson);
                        if (stateManager.MultiState.MultiChipBalance is not null)
                        {
                            ...
                            stateManager.Events.OnMultiChipBalance?.Invoke(
                                stateManager.MultiState.MultiChipBalance
                            );
                        }
                    ...
                    {
                        var dto = Utils.TryDeserialize<ChipBalanceIdleDto>(dataJson);
                        if (stateManager.MultiState.MultiChipBalance is not null)
                        {
                            stateManager.MultiState.MultiChipBalance = new ChipBalanceDto
                            {
            ...
            {
                Debug.LogError(
                    $"Failed to process event {eventName}: {ex.Message}. Json: {dataJson}"
                );
            }
        }
    }
