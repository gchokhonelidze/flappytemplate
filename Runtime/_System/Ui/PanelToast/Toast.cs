...
public class Toast : MonoBehaviour
{
	...
	private void Reset()
	{
		...
		hideTween?.Kill();
		mainTween?.Kill();
		rectTransform!.anchoredPosition = new Vector2(rectTransform.rect.width + offsetX, rectTransform.anchoredPosition.y);
	}
		...
		mainTween = DOTween
			...
			.OnComplete(() =>
			{
				if (currentOptions.AutoHide && currentOptions.HideAfter.TotalMilliseconds > 0)
					autoHideTween = DOVirtual.DelayedCall(currentOptions.HideAfter.Seconds, Hide);
			});
	}
	...
	public void Hide()
	{
		...
		hideTween = DOTween
			.To(
				() => rectTransform.anchoredPosition,
				x => rectTransform.anchoredPosition = x,
				new Vector2(rectTransform.rect.width + offsetX, rectTransform.anchoredPosition.y),
				animationDuration
			)
			.SetEase(Ease.InBack)
			.OnComplete(() =>
